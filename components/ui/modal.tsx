...
export default function Modal({ open, onClose, title, children, size = "md", footer }: ModalProps) {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" onClick={onClose}>
            ...
            <div
                className={cn("relative w-full bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl shadow-black/40 flex flex-col max-h-[90vh]", sizeMap[size])}
            ...
            >
                ...
                {/* Footer */}
                {footer && (
                    <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-slate-800/60 flex-shrink-0">
                        {footer}
                    </div>
                )}
            </div>
